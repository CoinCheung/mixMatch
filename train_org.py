...
torch.set_num_threads(1) # set cpu parallel thread number

def set_model():
    model = WideResnet(n_classes, k=1, n=28) # wide resnet-28
    model.train()
    model.cuda()
...
def evaluate(ema):
    ...
    dlval = get_val_loader(batch_size=128, num_workers=1, dataset='cifar10')
    matches = []
